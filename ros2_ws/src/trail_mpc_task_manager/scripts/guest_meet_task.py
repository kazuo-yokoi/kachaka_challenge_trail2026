    ...
    def execute_guest_meet(self) :
        ...
        for i in range(5) :
            ...
            if not answer["isThereAnyHuman"] :
                self.voice_manager.speak("人が認識されていません。カメラの前に立ってください。", wait=True)
            elif not answer["isHumansUpperBodyVisible"] :
                self.voice_manager.speak("カメラに近いです。カメラから離れてください。", wait = True)
            else : 
                # self.camera_manager.save_image(image)
                break

        answer = self.llm_manager.infer(["私のシャツ、ズボン、靴下を一行で日本語で説明してください。", pil_image])
        # self.get_logger().info(answer.text)
